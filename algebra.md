...
[Example](Vanishing ideal): Let $\mathbb{F}[X_1,\ldots,X_n]$ be a polynomial ring and let $\mathbf{z}=(z_1,\ldots,z_n)\in\mathbb{F}^n$, then the vanishing ideal of the singleton set ${\mathbf{z}}$ is $\lbrace f\in\mathbb{F}[X_1,\ldots,X_n]:f(\mathbf{z})=0\rbrace=(X_1-z_1,\ldots,X_n-z_n)$.
